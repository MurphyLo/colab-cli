...
{"type": "port_forward_create", "localHost": "127.0.0.1", "localPort": 7860, "remotePort": 7860}
...
{"type": "port_forward_created", "id": 1, "localHost": "127.0.0.1", "localPort": 7860, "remotePort": 7860, "proxyUrl": "https://7860-...colab.dev"}
{"type": "port_forward_list_result", "sessions": [{"id": 1, "localHost": "127.0.0.1", "localPort": 7860, "remotePort": 7860, "startedAt": "...", "proxyUrl": "..."}]}
...
| `terminal/terminal-buffer.ts` | 无对应 | **全新**：colab-vscode 直接操作 VS Code Terminal 面板（输出即时渲染，无缓冲需求）。colab-cli 因 daemon detach/attach 模式需要缓冲历史输出，使用有界环形缓冲区（100KB 上限，chunk-level 淘汰，支持 `tailBytes` 截取）。Snapshot 模式（`--no-wait`/`--tail`）额外经 `renderForSnapshot()` 后处理：剥离 ANSI escapes（含 DEC 私有模式序列）、模拟 `\r` 覆写折叠进度条输出、折叠连续空行并裁剪首尾空行，使非交互消费方（Agent 等）获得干净的可见文本而非原始终端字节流 |
...
```
curl http://127.0.0.1:7860            (用户浏览器 / 本地程序)
    │
    ▼
...
| `terminal/terminal-buffer.ts` | 无对应 | **全新**：有界环形缓冲区（默认 100KB），用于 daemon 侧缓存终端输出，支持 attach 时回放和 `--tail` 截取。Snapshot 模式下经 `renderForSnapshot()` 后处理（剥离 ANSI 含 DEC 私有模式序列、折叠 `\r` 覆写、折叠连续空行并裁剪首尾空行），为非交互消费方提供干净输出 |
