...
以下命令默认使用 `README.md` 中的标准 CLI 入口 `colab`。如果你是在仓库源码下本地开发，请先执行 `npm install && npm run build && npm link`。
...
L4 仅支持 `High-RAM`，应显式使用 `--shape high-ram`，不要传 `--shape standard`。
...
```bash
colab exec "%pip install pymupdf -q"
```
...
```bash
colab exec "%pip install 'numpy<2.1'"
```
...
| pip install 后新包不生效 | 仍然 import 到旧版本 | 必须执行 `colab runtime restart` 重启内核 |
